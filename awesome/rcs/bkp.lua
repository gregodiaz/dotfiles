...
-- awful.util.tagnames = {  " ", " ", " ", " ", " ", " ", " ", " ", " ", " "  }
--awful.util.tagnames = { " DEV ", " WWW ", " DOC ", " VBOX ", " CHAT ", " MUS ", " VID ", " GFX " }
awful.util.tagnames = { " 1.DEV ", " 2.WWW ", " 3.INIT ", " 4.SYS ", " 5.CHAT ", " 6.MUS " }
...
-- dmenu
    ...
    awful.key({ modkey, altkey }, "f",
        function() awful.util.spawn(terminal .. " -e sh ./.config/vifm/scripts/vifmrun") end,
    ...
    awful.key({ modkey, altkey }, "l",
        function() awful.util.spawn(terminal ..
                " -e lynx --cfg=~/.lynx/lynx.cfg --lss=~/.lynx/lynx.lss -vikeys gopher://distro.tube")
        end,
    ...
    -- screenshots
    awful.key({}, "Print",
        function() awful.util.spawn("scrot 'ArcoLinuxD-%Y-%m-%d-%s_screenshot_$wx$h.jpg' -e 'mv $f $$(xdg-user-dir PICTURES)'") end
        ,
    ...
    -- On the fly uclipess gaps change
    -- awful.key({ altkey, "Control" }, "j", function () lain.util.uclipess_gaps_resize(1) end,
    --           {description = "increment uclipess gaps", group = "tag"}),
    -- awful.key({ altkey, "Control" }, "l", function () lain.util.uclipess_gaps_resize(-1) end,
    --           {description = "decrement uclipess gaps", group = "tag"}),
    ...
    -- Standard program
    awful.key({ modkey, }, "t", function() awful.spawn(terminal .. " -e /usr/bin/zsh") end,
        { description = "Open terminal", group = "super" }),
    ...
    awful.key({ altkey, "Shift" }, "h", function() awful.tag.incncol(-7, nil, true) end,
        { description = "increase the number of columns", group = "layout" }),
    awful.key({ altkey, "Shift" }, "l", function() awful.tag.incncol(-1, nil, true) end,
        { description = "decrease the number of columns", group = "layout" }),

    awful.key({ modkey, }, "space", function() awful.layout.inc(1) end,
        { description = "clipect next", group = "layout" }),
    --awful.key({ modkey, "Shift"   }, "space", function () awful.layout.inc(-1)                end,
    -- {description = "clipect previous", group = "layout"}),
    ...
    -- Brightness
    awful.key({ modkey, "Control" }, "r", function() os.execute("xbacklight -inc 10") end,
        { description = "+10%", group = "hotkeys" }),
    awful.key({ modkey, "Control" }, "e", function() os.execute("xbacklight -dec 10") end,
        { description = "-10%", group = "hotkeys" }),
    ...
    -- awful.key({ modkey }, "c", function () awful.spawn.with_shell("xclip | xclip -i -b") end,
    --           {description = "copy terminal to gtk", group = "hotkeys"}),
    -- Copy clipboard to primary (gtk to terminals)
    -- awful.key({ modkey }, "v", function () awful.spawn.with_shell("xclip -b | xclip") end,
...
-- if #screen.connect_signal == 1 then
-- run_in_tab("firefox", " 2.WWW ")
-- run_in_tab("spotify", " 6.MUS ")
-- run_in_tab("whatsdesk", " 6.MUS ")
-- else
--     local primary = screen.primary
--     if primary.index == 1 then
--         run_in_tab("firefox", " 2.WWW ")
--     elseif primary.index == 2 then
--         run_in_tab("firefox", " 2.WWW ")
--         run_in_tab("whatsdesk", " 6.CHAT ")
--         run_in_tab("spotify", " 6.MUS ")
--     end
-- end

-- Seleccionar una imagen al azar de la carpeta "wallpapers"
local wallpapers_folder = os.getenv("HOME") .. "/wallpapers"
local files = {}
for file in io.popen('find ' .. wallpapers_folder .. ' -type f -name "*.jpg"'):lines() do
    files[#files + 1] = file
end
-- if #files > 0 then
local wallpaper = files[math.random(#files)]
--     -- Establecer la imagen como fondo de pantalla
--     awful.spawn.with_shell("nitrogen --set-scaled " .. wallpaper)
-- else
--     -- No se encontraron imágenes en la carpeta especificada
--     print("No se encontraron imágenes en la carpeta" .. wallpapers_folder)
-- end
...
for s = 2, screens do
    if s == 1 then
        ...
        run_in_tab("firefox", " 2.WWW ")
        run_in_tab("spotify", " 6.MUS ")
        run_in_tab("whatsdesk", " 5.CHAT ")
    elseif s == 2 then
        -- Estoy en la pantalla 2
        run_in_tab("firefox", " 2.WWW ")
        run_in_tab("spotify", " 6.MUS ")
    end
end
...
if beautiful.wallpaper then
    for s = 1, screen.count() do
        ...
        if s < 2 then
            gears.wallpaper.maximized(wallpapers_folder.."/0001.jpg", s, true)
        else
            gears.wallpaper.maximized(wallpapers_folder.."/0002.jpg", s, true)
        end
    end
end
-- }}}
