...
local reloader = function()
	if SHOULD_RELOAD_TELESCOPE then
		RELOAD "plenary"
		RELOAD "telescope"
		RELOAD "greg.telescope.setup"
	end
end

local fb_actions = require("telescope").extensions.file_browser.actions
local actions = require "telescope.actions"
local my_actions = require "greg.telescope.actions"
...
function M.branches()
	require("telescope.builtin").git_branches {
		prompt_title = "< Git Branches > [ C-j: create ]",
...
M.find_nvim_plugin = function()
	require("telescope.builtin").find_files {
		prompt_title = "Plugins > [ t: create, d: disable, e: enable ]",
...
function M.gateway()
	return require("greg.php.gateway").graphql_definitions {
		layout_config = {
			horizontal = {
