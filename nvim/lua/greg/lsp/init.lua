return {
	setup = function(ops)
		ops = vim.tbl_deep_extend("force", require "greg.lsp.default", ops)
		...
		for server, options in pairs(ops.servers) do
			if options.enable == true then
				local ok, definition = pcall(require, "greg.lsp.servers." .. server)
		...
		-- TODO: implement none ls with options
		-- require "greg.lsp.none-ls"
		...
		vim.api.nvim_create_user_command("GregLSP", function()
			vim.ui.select(servers, { prompt = "LSP Servers" }, function(server)
				...
				local path = string.format("%s/lua/greg/lsp/servers/%s.lua", vim.fn.stdpath "config", server)
